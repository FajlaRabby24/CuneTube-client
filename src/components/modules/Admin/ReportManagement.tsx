...
const ReportManagement = ({ initialQueryString }: ReportManagementProps) => {
  ...
  const handleViewDetails = (report: IReport) => {
  ...
  const handleResolve = (report: IReport) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        resolveMutation.mutate({ reportId: report.id, resolution: result.value });
        if (isDetailsOpen && selectedReport?.id === report.id) setIsDetailsOpen(false);
      }
    });
  };
            ...
            {reports.length === 0 ? (
              ...
              reports.map((report: IReport) => (
                <TableRow key={report.id}>
                  ...
                  <TableCell>
                    <span className="text-sm">{report.reason.replace(/_/g, " ")}</span>
                  </TableCell>
          ...
          {selectedReport && (
            <div className="space-y-6">
              ...
              <div className="grid gap-2 border rounded-md p-4">
                <h4 className="font-semibold text-sm">Reported target: {selectedReport.targetType}</h4>
                <div className="p-3 bg-muted/50 rounded-md text-sm border">
                  <p className="font-medium text-xs mb-1 text-muted-foreground">Original Content Author: {selectedReport.review?.user.name || selectedReport.comment?.user.name || "Unknown"} </p>
                  <p>
              ...
              <div className="grid gap-2 border rounded-md p-4">
                ...
                <div className="text-sm">
                  <span className="font-medium inline-block min-w-24">Reason:</span> 
                  {selectedReport.reason.replace(/_/g, " ")}
                </div>
                {selectedReport.description && (
                  <div className="text-sm mt-2">
                    <span className="font-medium block mb-1">Additional User Description:</span>
                    <div className="bg-muted/50 p-2 rounded whitespace-pre-wrap">{selectedReport.description}</div>
                  </div>
                )}
              </div>
              ...
              <div className="grid gap-2 border rounded-md p-4">
                ...
                <div className="text-sm flex items-center gap-2 mb-2">
                  <span className="font-medium inline-block min-w-24">Current state:</span> 
                  {getStatusBadge(selectedReport.status)}
                </div>

                {selectedReport.status !== "PENDING" && (
                  <div className="text-sm space-y-1">
                    <div><span className="font-medium inline-block min-w-24">Resolved At:</span> {selectedReport.resolvedAt ? new Date(selectedReport.resolvedAt).toLocaleString() : "N/A"}</div>
                    {selectedReport.resolution && (
                      <div><span className="font-medium inline-block min-w-24">Resolution:</span> {selectedReport.resolution}</div>
                    )}
                  </div>
                )}
              </div>
              ...
              {selectedReport.status === "PENDING" && (
                <div className="flex gap-4 justify-end pt-4">
                  <Button variant="outline" onClick={() => handleDismiss(selectedReport)}>
                    Dismiss Report
                  </Button>
                  <Button variant="destructive" onClick={() => handleResolve(selectedReport)}>
                    Resolve & Take Action
                  </Button>
                </div>
              )}
